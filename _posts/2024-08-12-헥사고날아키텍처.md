...
```
├─ adapter            # 외부와의 인터페이스 역할을 수행하는 어댑터 계층
│   ├─ in             # 입력 어댑터 (예: 컨트롤러, 이벤트 리스너 등)
│   │   ├─ model      # 입력 데이터 모델을 위한 폴더
│   │   │   └─ request # 입력 요청(Request) 모델들
│   └─ out            # 출력 어댑터 (예: 데이터베이스, 외부 API 등)
│       └─ persistence # 영속성 계층과 연결된 어댑터
│           └─ entity # 데이터베이스 엔티티 클래스들
│
├─ application        # 애플리케이션 서비스 계층, 유즈케이스 구현
│   ├─ port           # 포트 인터페이스를 정의하는 폴더
│   │   ├─ in         # 입력 포트 (예: 유즈케이스 인터페이스)
│   │   └─ out        # 출력 포트 (예: 저장소 인터페이스)
│   └─ service        # 유즈케이스에 따라 비즈니스 로직을 처리하는 서비스 클래스들
│
├─ domain             # 순수 비즈니스 로직과 핵심 도메인 모델을 포함하는 계층
│   └─ model          # 도메인 모델 (애그리게잇, 엔티티, 값 객체 등)
│
└─ mapper             # 도메인 객체와 DTO 간 매핑을 위한 매퍼
```